...
工具不會把你的檔案送到任何地方。程式碼裡沒有 `fetch`、沒有 `XMLHttpRequest`、沒有後端。需要跨次記憶的 zine-fold 設定留在 `localStorage`、頁面圖片留在 `IndexedDB`；id-photo-sheet 連照片都不保存，關閉分頁即清除；receipt-maker 只把抬頭、稅率這類文件設定寫進 `localStorage`，姓名、身分證字號、地址與帳號不留存；speaker-timer 只記演講長度與提醒音偏好，提醒音是瀏覽器即時合成的，沒有音檔。這些資料都只存在使用者自己的瀏覽器裡。
...
自己驗證：開發者工具 Network 分頁重新整理頁面，除了頁面本身不該出現任何請求；或在 Console 執行 `performance.getEntriesByType('resource')`，應該回傳空陣列。
