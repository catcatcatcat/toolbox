...
工具不會把你的檔案送到任何地方。程式碼裡沒有 `fetch`、沒有 `XMLHttpRequest`、沒有後端。需要跨次記憶的 zine-fold 設定留在 `localStorage`、頁面圖片留在 `IndexedDB`；id-photo-sheet 連照片都不保存，關閉分頁即清除；receipt-maker 只把抬頭、稅率這類文件設定寫進 `localStorage`，姓名、身分證字號、地址與帳號不留存。這些資料都只存在使用者自己的瀏覽器裡。
